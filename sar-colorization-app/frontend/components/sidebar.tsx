...
import { Activity, BarChart3, Boxes, FileText, Github, GitCompareArrows, Info, Moon, Orbit, ScanSearch, Sparkles } from "lucide-react";
...
export function Sidebar() {
  ...
  return <aside className="sticky top-0 flex h-dvh w-full shrink-0 flex-col border-b border-white/[.08] bg-[#0c0c0e]/90 p-5 backdrop-blur-xl lg:w-72 lg:border-b-0 lg:border-r">
    ...
    <div className="my-7 border-t border-white/[.08]"/><div className="px-2"><p className="eyebrow">Model status</p><div className="mt-3 space-y-2">{["pix2pix", "sarfusionformer", "color_corrector"].map(name => { const item = health.data?.models[name]; return <div key={name} className="glass flex items-center gap-2 rounded-xl px-3 py-2.5 text-xs"><Activity size={14} className={item?.available ? "text-emerald-400" : "text-zinc-600"}/><span className="capitalize text-zinc-300">{name.replace("_", " ")}</span><span className="ml-auto text-zinc-500">{item?.available ? "Ready" : health.isLoading ? "Checking" : "Offline"}</span></div>; })}</div></div>
