...
export function Sidebar() {
  ...
  return <aside className="sticky top-0 flex h-dvh w-full shrink-0 flex-col border-b border-white/[.08] bg-[#0c0c0e]/90 p-5 backdrop-blur-xl lg:w-72 lg:border-b-0 lg:border-r">
    <Link href="/pix2pix" className="brand-card group"><span className="brand-logo"><Orbit size={25}/></span><span className="brand-copy"><span className="brand-label">Earth Observation AI</span><strong className="brand-title"><span>Geo</span><span className="brand-title-accent">Vision</span></strong></span></Link>
    <nav className="mt-7 space-y-1">{navigation.map(({ href, label, icon: Icon }) => <Link key={href} href={href} className={cn("flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm transition", path === href ? "bg-sky-400/10 text-sky-200" : "text-zinc-400 hover:bg-white/[.05] hover:text-white")}><Icon size={17}/>{label}</Link>)}</nav>
